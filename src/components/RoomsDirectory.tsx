  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-3xl border border-indigo-500/20">
        ...
        <p className="text-xs text-slate-400 mt-1">دليل القاعات التدريبية والاجتماعات المتاحة بالشركة والتجهيزات التقنية المتاحة بكل قاعة</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {rooms.map((room) => {
          ...
          return (
            <div
              key={room.id}
              className="glass-panel glass-panel-hover p-6 rounded-3xl border border-slate-700/60 flex flex-col justify-between"
            >
              <div>
                {/* Header */}
                ...
                {/* Meta details */}
                <div className="grid grid-cols-2 gap-3 py-4 text-xs text-slate-300">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(room.amenities || []).map((eq) => {
                      ...
                      return (
                        <span
                          key={eq}
                          className="px-2.5 py-1 rounded-xl bg-slate-900/80 border border-slate-700/60 text-slate-300 text-[11px] font-medium flex items-center gap-1.5"
                        >
              ...
              <div className="pt-6 mt-6 border-t border-slate-800">
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-indigo-600/30 hover:bg-indigo-600 border border-indigo-500/50 hover:border-indigo-500 text-indigo-200 hover:text-white text-xs font-bold transition-all shadow-md"
                >
                  حجز هذه القاعة الآن
                </button>
              </div>
